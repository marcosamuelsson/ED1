...
class Arvore:
    ...
    def pertence(self, v):
        if self.__pertence(self.raiz, v) == True:
            print("O valor selecionado pertence à árvore")
            return True
        else:
            print("O valor selecionado não pertence à árvore")
            return False
    ...
    def altura(self):
        print("A altura dessa árvore é:", self.__altura(self.raiz))
    ...
    def folhas(self):
        print("A quantidade de folhas dessa árvore é: ", self.__folhas(self.raiz))
    
    def __folhas(self, raiz):
        v = 0
        if raiz != None:
            v = v + 1 
        if raiz == None:
            return 0 
        return self.__folhas(raiz.prim) + self.__folhas(raiz.prox) + v
        

    def pares(self):
        print("A quantidade de números pares nessa lista é: ",self.__pares(self.raiz))
    
    def __pares(self, raiz):
        v = 0
        if raiz == None:
            return 0 
        if raiz.info % 2 == 0:
            v = v + 1
        return self.__pares(raiz.prim) + self.__pares(raiz.prox) + v
